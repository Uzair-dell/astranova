...
pub fn match_pattern(template: &Expr, target: &Expr) -> Option<Env> {
    match (template, target) {
        ...
        // ---------- Literals ----------
        (Expr::Number(a), Expr::Number(b)) if (a - b).abs() < f64::EPSILON => Some(HashMap::new()),
        ...
        // ---------- Sum ----------
        (
            Expr::Sum { index: idx_t, start: s_t, end: e_t, body: b_t },
            Expr::Sum { index: idx_a, start: s_a, end: e_a, body: b_a },
        ) if idx_t == idx_a => {
        ...
        // ---------- Cross‑match: BinaryOp with Pow operator ↔ Pow node ----------
        (Expr::BinaryOp { op: BinOp::Pow, left: l_t, right: r_t }, Expr::Pow { base: b_a, exp: e_a }) => {
            let env_base = match_pattern(l_t, b_a)?;
            let env_exp  = match_pattern(r_t, e_a)?;
            merge_envs(env_base, env_exp)
        }
        (Expr::Pow { base: b_t, exp: e_t }, Expr::BinaryOp { op: BinOp::Pow, left: l_a, right: r_a }) => {
            let env_base = match_pattern(b_t, l_a)?;
            let env_exp  = match_pattern(e_t, r_a)?;
            merge_envs(env_base, env_exp)
        }

        // ---------- UnaryOp ----------
        (
            Expr::UnaryOp { op: op_t, operand: o_t },
            Expr::UnaryOp { op: op_a, operand: o_a },
...
/// Replace every placeholder variable in `expr` with the captured sub‑expression.
pub fn instantiate(expr: &Expr, env: &Env) -> Expr {
    match expr {
        Expr::Variable(name) if name.starts_with("__") => {
            env.get(name).cloned().unwrap_or(Expr::Number(0.0))
        }
...
// ---------- JSON helper routines (unchanged) ----------
